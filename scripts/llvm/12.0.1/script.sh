...
# LLVM 12.0.1 - Uses monorepo structure (starting from LLVM 12+)
...
function mason_load_source {
    ...
    # LLVM 12+ uses a single monorepo tarball
...
function mason_prepare_compile {
    ...
    if [[ -z "${CUSTOM_CC:-}" ]] && [[ -z "${CUSTOM_CXX:-}" ]]; then
        mason_error "LLVM 12.0.1 requires a C++14 compiler."
...
function mason_compile {
    ...
    # Key difference: For monorepo (LLVM 12+), we need to specify which projects to build
