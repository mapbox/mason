...
# LLVM 13.0.1 - Uses monorepo structure (different from older LLVM versions)
...
function mason_load_source {
    ...
    # LLVM 13 uses a single monorepo tarball
...
function mason_prepare_compile {
    ...
    if [[ -z "${CUSTOM_CC:-}" ]] && [[ -z "${CUSTOM_CXX:-}" ]]; then
        mason_error "LLVM 13.0.1 requires a C++14 compiler."
