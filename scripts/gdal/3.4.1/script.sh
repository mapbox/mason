...
function mason_load_source {
    mason_download \
        ...
        b96ea7ef832fb87743ecbfa65d58bb8310eed8e9
...
function mason_prepare_compile {
    ...
    perl -i -p -e "s/${FIND}/${REPLACE}/g;" ${MASON_LIBCURL}/lib/libcurl.la
...
function mason_compile {
    ...
    export CFLAGS="${CFLAGS} -O3 -DNDEBUG -Wl,-Bstatic"
    export CXXFLAGS="${CXXFLAGS} -O3 -DNDEBUG"

    CUSTOM_LIBS="-L${MASON_GEOS}/lib -lgeos_c -lgeos -L${MASON_SQLITE}/lib -lsqlite3 -L${MASON_TIFF}/lib -ltiff -L${MASON_JPEG}/lib -ljpeg -L${MASON_PROJ}/lib -lproj -L${MASON_PNG}/lib -lpng -L${MASON_EXPAT}/lib -lexpat -L${MASON_LIBCURL}/lib -lcurl"
    CUSTOM_CFLAGS="${CFLAGS} -I${MASON_GEOS}/include -I${MASON_LIBPQ}/include -I${MASON_TIFF}/include -I${MASON_JPEG}/include -I${MASON_PROJ}/include -I${MASON_PNG}/include -I${MASON_EXPAT}/include -I${MASON_LIBCURL}/include"
    ...
    if [[ $(uname -s) == 'Linux' ]]; then
        ...
        CUSTOM_LDFLAGS="${LDFLAGS} -L${MASON_LIBPQ_PATH}"
    ...
    if [[ $(uname -s) == 'Linux' ]]; then
        CUSTOM_LIBS="-L${MASON_LIBPQ}/lib/libpq.a -pthread ${CUSTOM_LIBS}"
    fi
    ...
    # PKG_CONFIG_PATH=${MASON_LIBPQ}/lib/pkgconfig \
    LIBS="${CUSTOM_LIBS}" LDFLAGS="${CUSTOM_LDFLAGS}" CFLAGS="${CUSTOM_CFLAGS}" \
     ./configure \
        ...
        --with-spatialite=no \
        --with-curl=${MASON_LIBCURL}/bin/curl-config \
        --with-xml2=no \
        --with-pcraster=no \
        --with-cfitsio=no \
        --with-odbc=no \
